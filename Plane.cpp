...
bool Plane::isInside(glm::vec3 pt)
{
    ...
    glm::dot(glm::cross(ud, vd), this->normal(a)) > 0;
}
...
glm::vec3 Plane::normal(glm::vec3 pt)
{
    ...
    n = normalize(glm::cross((b-a), (d-a)));

    return n;
}


// Override get colour method, for checkered floors


glm::vec3 Plane::getColor (glm::vec3 pt)
{
    if (chequered) {
        if (abs((int) floor(pt.x / 5) % 2) == abs((int) floor(pt.z / 5) % 2)) {
            return color;
        } else {
